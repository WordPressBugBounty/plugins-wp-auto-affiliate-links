...
function wpaal_exclude_page_controller() {
    $default_tab = 'exclude_posts';
    $active_tab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : $default_tab;
    
    $base_url = admin_url('admin.php?page=aal_exclude_posts');

    ?>
    <div class="wrap">  
        <div class="icon32" id="icon-options-general"></div>  
        <h2>Exclude Content from Auto Affiliate Links</h2>
        <br />

        <h2 class="nav-tab-wrapper">
            <a href="<?php echo $base_url; ?>&tab=exclude_posts" class="nav-tab <?php echo $active_tab == 'exclude_posts' ? 'nav-tab-active' : ''; ?>">Exclude Posts/Pages</a>
            <a href="<?php echo $base_url; ?>&tab=exclude_date" class="nav-tab <?php echo $active_tab == 'exclude_date' ? 'nav-tab-active' : ''; ?>">Exclude by Date</a>
            <a href="<?php echo $base_url; ?>&tab=exclude_author" class="nav-tab <?php echo $active_tab == 'exclude_author' ? 'nav-tab-active' : ''; ?>">Exclude by Author</a>
        </h2>

        <div class="aal-tab-content" style="margin-top: 20px;">
            <?php
            switch ($active_tab) {
                case 'exclude_date':
                    wpaal_exclude_dates_tab_ui();
                    break;
                case 'exclude_author':
                    if (function_exists('wpaal_exclude_authors_ui')) {
                        wpaal_exclude_authors_ui();
                    } else {
                        echo '<p>Author exclusion module is missing.</p>';
                    }
                    break;
                case 'exclude_posts':
                default:
                    wpaal_exclude_posts_tab_ui();
                    break;
            }
            ?>
        </div>
...
/**
 * TAB-UL 1: EXCLUDE BY POSTS (Căutare, ID, URL)
 */
function wpaal_exclude_posts_tab_ui() {
    // Acțiunea de flush (curățare completă)
    ...
    <div id="aal_search_exclude_container">
        ...
        <div id="aal_search_results_container" style="display:none;">
            <select id="aal_search_results" multiple="multiple" style="width: 100%; max-width: 400px; height: 150px;"></select>
            <br/>
    ...
    <form class="aal_exclude_posts">
        ...
        if($aal_exclude_posts_array[0]) {
            ...
            <div class='aal_excludeditemheader'>
                <div class='aal_excludedcol aal_excludedidcol'>Post ID</div>
                <div class='aal_excludedcol aal_excludedtitle'>Post/Page Title</div>
                <div class='aal_excludedcol'>Status</div>
                <div class='aal_excludedcol'>Actions</div>
            </div>
            ...
            foreach ($aal_exclude_posts_array as $aal_exclude_post_id) {
                if($aal_exclude_post_id != '') { 
                    ...
                    $status = !$exclude_title ? 'post with the given id does not exist' : get_post_status($aal_exclude_post_id);
                    if($status == 'publish') { $status = 'Published'; } // Fixed = to == here for comparison
...
/**
 * TAB-UL 2: EXCLUDE BY DATE
 */
function wpaal_exclude_dates_tab_ui() {
    ?>
